...
def orderhumidity():
        content_variable = open('humidity_data.csv', "r") # Open to read the CSV we generate from the main
        ...
        humidity = int(l[1]) # Convert the 2nd value of the list into an integer which correspond to the humidity

        order = 0 # If we don't take any action

        if humidity < 30 : # If we need to irrigate
                order = 1
        return order
...
def main():
	# This two variables are going to be used in the humidity CSV
        data_humidity = 0 # humidity data we're collecting from the feed humidity
        date_csv = 0 # time of when we're collecting the humidity data

        while data_humidity != -1: # While there's still data to collect in the feed
                data_humidity = getvalue() # Collecting the value, which is a dictionnary
                if data_humidity != -1: # If there's still data
                        #print(data.value)
                        #print(data.created_at)
                        valeur = str(data_humidity.value) # Convert the humidity value into a string
                        date_csv = data_humidity.created_at # Collecting the time of when the data is created in the feed
                        ...
                        c.writerow([date_csv, valeur]) # Writing the humidity value and its time creation
